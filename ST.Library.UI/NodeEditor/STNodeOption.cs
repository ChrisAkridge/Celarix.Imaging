...
namespace ST.Library.UI.NodeEditor
{
    public class STNodeOption
    {
        ...
        public virtual ConnectionStatus CanConnect(STNodeOption op) {
            ...
            if (op.Owner == _Owner) return ConnectionStatus.SameOwner;
            if (_Owner.LockOption || op._Owner.LockOption) return ConnectionStatus.Locked;
            if (_IsSingle && m_hs_connected.Count == 1) return ConnectionStatus.SingleOption;
            if (op.IsInput && STNodeEditor.CanFindNodePath(op.Owner, _Owner)) return ConnectionStatus.Loop;
